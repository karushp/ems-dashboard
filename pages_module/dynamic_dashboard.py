...
def display_building_analysis_tab(df, region, industry):
    ...
    # Building Size vs Energy Consumption Correlation
    st.markdown("### Building Size vs Energy Consumption Analysis")

    # Calculate total floor area for each building
    floor_area_cols = [col for col in df.columns if "Floor Area" in col]
    if floor_area_cols:
        # Create a building identifier (using Source File as proxy for building)
        building_data = (
            df.groupby("Source File")
            .agg(
                {
                    "Total": "mean",  # Average energy consumption per building
                    **{
                        col: "first" for col in floor_area_cols
        ...
        # Calculate total floor area for each building
        building_data["Total_Floor_Area"] = building_data[floor_area_cols].sum(axis=1)

        # Filter out buildings with zero floor area
        building_data = building_data[building_data["Total_Floor_Area"] > 0]
        ...
        if len(building_data) > 0:
            ...
            fig_scatter.add_trace(
                px.scatter(
                    ...
                    y="Total",
                    trendline="ols",
                    trendline_color_override=CHART_COLORS["secondary"],
                ).data[
                    1
                ]  # Get the trendline trace
            )

            fig_scatter = apply_dark_gray_labels(fig_scatter)
            st.plotly_chart(fig_scatter, width="stretch", key="building_size_energy")

            # Calculate correlation
            correlation = building_data["Total_Floor_Area"].corr(building_data["Total"])
            st.info(
                f"**Correlation Coefficient**: {correlation:.3f} - {'Strong' if abs(correlation) > 0.7 else 'Moderate' if abs(correlation) > 0.3 else 'Weak'} correlation between building size and energy consumption"
            )

    # Building Characteristics Heatmap
    st.markdown("### Building Characteristics Correlation Heatmap")
    ...
    building_type_cols = [col for col in df.columns if "Building Type" in col]
    for col in building_type_cols:
        binary_features[col] = heatmap_data[col]

    # Floor area features
    floor_area_cols = [col for col in df.columns if "Floor Area" in col]
    for col in floor_area_cols:
        binary_features[col] = heatmap_data[col]

    # Contract power features
    contract_cols = [col for col in df.columns if "Contract Power" in col]
    for col in contract_cols:
        binary_features[col] = heatmap_data[col]

    # Add other relevant features
    if "building_has_freezer" in df.columns:
        binary_features["building_has_freezer"] = heatmap_data["building_has_freezer"]

    if "is_weekend" in df.columns:
        binary_features["is_weekend"] = heatmap_data["is_weekend"]
    ...
    for component in energy_components:
        if component in df.columns:
            binary_features[component] = heatmap_data[component]

    if len(binary_features) > 1:
        # Create correlation matrix
        correlation_df = pd.DataFrame(binary_features).corr()

        # Create heatmap
        fig_heatmap = px.imshow(
            correlation_df,
            title="Building Characteristics Correlation Heatmap",
        ...
        fig_heatmap.update_layout(
            xaxis_title="Features", yaxis_title="Features", font=dict(color="#333333")
        )

        st.plotly_chart(fig_heatmap, width="stretch", key="building_heatmap")

        # Show insights
        st.info(
            "**Heatmap Insights**: Red indicates positive correlation, blue indicates negative correlation. Values closer to 1 or -1 show stronger relationships between building characteristics."
        )
